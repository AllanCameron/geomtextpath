...
The text will always stick together and follow the path, even if you change the size and aspect ratio of your plotting device. It does so without the need to redraw your plot each time, as shown in the [introduction](https://allancameron.github.io/geomtextpath/articles/geomtextpath.html) vignette.
...
Many paths will be too noisy or too angular to directly label in a visually appealing fashion if the text adheres too closely to the intricacies of the line. Often, a `geom_textsmooth` with `include_line = FALSE` is the best option in such cases, as in the examples above. There is also a `keep_straight` parameter so that a label is still applied at an appropriate point and angle on the line, but the text will not attempt to follow every bump on the path.
